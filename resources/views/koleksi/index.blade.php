    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-7 rounded-[30px] border border-white shadow-sm flex items-center justify-between">
            <div>
                ...
                <h3 class="text-3xl font-[900] text-slate-800">
                    {{ \App\Models\Koleksi::where('user_id', Auth::id())->count() }}
                </h3>
            </div>
            <div class="w-12 h-12 bg-blue-50 text-blue-400 rounded-2xl flex items-center justify-center">
                <i data-lucide="layers"></i>
            </div>
        </div>

        {{-- Kartu: Kategori Favorit --}}
        <div class="bg-white p-7 rounded-[30px] border border-white shadow-sm flex items-center justify-between">
            <div>
            ...
            <div class="w-12 h-12 bg-pink-50 text-pink-400 rounded-2xl flex items-center justify-center">
                <i data-lucide="heart"></i>
            </div>
        </div>

        {{-- Kartu: Rekomendasi AI (bisa diklik) --}}
        <a href="{{ route('koleksi.ai') }}"
           class="bg-white p-7 rounded-[30px] border border-white shadow-sm flex items-center justify-between hover:border-purple-200 hover:shadow-md transition-all duration-200 group">
            <div>
                <p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-1">Rekomendasi AI</p>
                <p class="text-xs font-bold text-purple-400 group-hover:text-purple-600 transition-colors">
                    Klik untuk analisis hobimu ✨
                </p>
            </div>
            <div class="w-12 h-12 bg-purple-50 text-purple-400 rounded-2xl flex items-center justify-center group-hover:bg-purple-100 transition-colors">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <form action="{{ route('koleksi.index') }}" method="GET" class="flex items-center gap-2">
                    ...
                    <button type="submit"
                            class="px-5 py-3 bg-[#FF8FAB] hover:bg-[#ff7b9e] text-white rounded-2xl text-xs font-black uppercase tracking-wider transition-all">
                        Cari
                    </button>

                    @if(request('search'))
                        <a href="{{ route('koleksi.index') }}"
                           class="px-5 py-3 bg-slate-50 hover:bg-slate-100 text-slate-500 rounded-2xl text-xs font-black uppercase tracking-wider transition-all">
                            Reset
                        </a>
                    @endif
                </form>
                ...
                <a href="{{ route('koleksi.create') }}"
                   class="px-5 py-3 bg-white hover:bg-pink-50 text-[#FF8FAB] rounded-2xl text-xs font-black uppercase tracking-wider flex items-center justify-center gap-1 border border-slate-100 shadow-sm transition-all">
                    + Tambah Baru
                </a>
            </div>
        </div>
                ...
                <div class="bg-white p-4 rounded-[30px] border border-slate-100 shadow-sm space-y-4 group relative">
                    ...
                    <div class="px-2 pb-2">
                        <div class="flex items-center justify-between mb-3">
                            <div>
                                <p class="text-[9px] font-black text-pink-400 uppercase tracking-widest mb-0.5">
                                    {{ $item->kategori }}
                                </p>
                                <h5 class="font-bold text-slate-800 text-base tracking-tight">
                                    {{ $item->nama }}
                                </h5>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                {{-- Lihat detail --}}
                                <a href="{{ route('koleksi.show', $item) }}"
                                   class="w-8 h-8 rounded-xl bg-slate-50 hover:bg-pink-50 flex items-center justify-center text-slate-400 hover:text-pink-400 transition-all">
                                    <i data-lucide="eye" class="w-3.5 h-3.5"></i>
                                </a>

                                {{-- Edit --}}
                                <a href="{{ route('koleksi.edit', $item) }}"
                                   class="w-8 h-8 rounded-xl bg-slate-50 hover:bg-blue-50 flex items-center justify-center text-slate-400 hover:text-blue-400 transition-all">
                                    <i data-lucide="pencil" class="w-3.5 h-3.5"></i>
                                </a>

                                {{-- Hapus --}}
                                <form method="POST" action="{{ route('koleksi.destroy', $item) }}"
                                      onsubmit="return confirm('Hapus koleksi ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            class="w-8 h-8 rounded-xl bg-slate-50 hover:bg-red-50 flex items-center justify-center text-slate-400 hover:text-red-400 transition-all">
            ...
            @empty
                ...
                <div class="col-span-3 text-center py-20 text-slate-300">
                    ...
                    @if(request('search'))
                        ...
                        <a href="{{ route('koleksi.index') }}"
                           class="mt-4 inline-block px-6 py-2.5 bg-slate-100 text-slate-500 text-xs font-black rounded-2xl">
                            Reset Pencarian
                        </a>
                    @else
                        <p class="text-sm font-bold">Belum ada koleksi. Yuk tambah!</p>
                        <a href="{{ route('koleksi.create') }}"
                           class="mt-4 inline-block px-6 py-2.5 bg-[#FF8FAB] text-white text-xs font-black rounded-2xl">
                            + Tambah Sekarang
                        </a>
                    @endif
                </div>
